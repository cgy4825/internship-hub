...
## 阶段 11：一键脚本（日常打开与更新）（2026-09-06）

### 前置条件
- 用户希望「不想自己动手、打开网站/更新数据都最简单」。

### 目标
提供双击即用的脚本，免手动敲命令。

### 产出
- `打开网站.bat`：双击 → 启动本地网站服务器 → 自动打开浏览器（`http://localhost:5300`）。
  处理首次运行自动 `npm install`、端口就绪检测。
- `更新数据.bat`：双击 → 抓取最新校招/实习数据 → 提交 git → 推送 GitHub。
- `使用指南.md`：面向非技术用户的中文操作说明。
- README 更新快速启动说明。

### 关键决策（重要）
- `.bat` 脚本文本**改用纯英文/ASCII**，避免 Windows cmd 对中文（UTF-8 无 BOM）按 GBK 解析
  导致的命令错乱（实测中文注释会破坏解析）。脚本逻辑本身正确，已用 cmd 验证。
- 用户日常流程即：**想用 → 双击「打开网站.bat」；想让岗位变新 → 双击「更新数据.bat」**。

### 验证方式
- `打开网站.bat`：cmd 运行后 5300 返回 HTTP 200，能自动启动 dev server。
- `更新数据.bat`：完整流程跑通——抓取（46 条）→ 提交 → 推送 GitHub（`e27cd77..ba1d7be`）。
- 两个脚本均不再因中文编码解析失败。

### 遗留问题
- 网站为本地运行，未部署成公网网址；如需手机/异地访问需部署（见 roadmap）。
