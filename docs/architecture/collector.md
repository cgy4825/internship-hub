...
- **云端采集可靠性**：采集器纯标准库（可在 GitHub Actions 云端运行）；
  `_fetch` 支持**重试（最多 3 次）**与 **UA 回退（移动端→桌面端）**，提升对反爬/网络抖动的成功率。
- 服务端渲染结构如有变动，可能影响解析；需定期验证采集是否稳定。
- 对反爬、网站改版导致的失败做好捕获，记录并提示（工作流在抓到 0 条时创建 GitHub Issue 告警）。

## 8. 每日自动更新

- `.github/workflows/collect-daily.yml`：
  - **每天 08:00 (UTC)** 定时采集；
  - **推送到 `main` 时也立即采集一次**（数据更新更及时）；
  - 抓取 → 生成 JSON/RSS → 自动提交回仓库（无变更则跳过）；
  - 抓到 0 条时退出码置 1，工作流创建 GitHub Issue 告警。
- 注意的是，采集工作流在 **GitHub 云端（境外）** 运行；对国内网站的抓取成功率取决于对方是否放行云端 IP/代理，存在一定不确定性。
